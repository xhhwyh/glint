...
impl Grid {
    ...
    pub fn visible_rows(&self) -> impl Iterator<Item = &crate::row::Row> {
        ...
        self.scrollback
            ...
            // When the scrollback offset is larger than the visible row count, show as many
            // scrollback rows as the viewport can hold and avoid underflowing the row count.
            .take(rows_len)
            .chain(
                self.rows
                    .iter()
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            if self.scrollback_len > 0 && self.scroll_region_saves_scrollback() {
    ...
    fn scroll_region_saves_scrollback(&self) -> bool {
        // Inline TUIs such as Codex insert finalized history by scrolling a region that starts at
        // the top row while leaving a live viewport below it. Preserve those top-pushed rows.
