...
impl ToolRegistry {
    ...
    fn tools(&self) -> Vec<&'static dyn ToolBehavior> {
        if self.subagent {
            return vec![&READ_TOOL, &GLOB_TOOL, &GREP_TOOL, &LSP_TOOL, &BASH_TOOL];
        }
        vec![
            &READ_TOOL,
            &GLOB_TOOL,
...
static ALL_TOOLS: [&dyn ToolBehavior; 12] = [
    &READ_TOOL,
    &GLOB_TOOL,
    &GREP_TOOL,
    &LSP_TOOL,
...
mod tests {
    ...
    #[test]
    fn task_request_channel_routes_task_controls() {
    ...
    fn subagent_registry_exposes_limited_tool_surface() {
        ...
        assert_eq!(names, ["Read", "Glob", "Grep", "LSP", "Bash"]);
